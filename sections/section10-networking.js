    ...
    questionSets: {
        ...
        set2: [
            ...
            },
            {
                ...
                allowedPreChecks: [
                    { command: "nmcli", requiredValues: ["connection", "show"] },
                    { command: "nmcli", requiredValues: ["connection", "show", "office"] }
                ],
            ...
            },
            {
                id: 4,
                category: "Implementation",
                description: "Set IPv4 method to manual for office connection",
                expected: [
                    { command: "nmcli", requiredValues: ["connection", "modify", "office", "ipv4.method", "manual"] },
                    { command: "nmcli", requiredValues: ["con", "mod", "office", "ipv4.method", "manual"] }
                ],
                allowedPreChecks: [],
                explanation: "Setting method to manual enables static IP configuration.",
                points: 1
            },
            {
                id: 5,
                category: "Implementation",
                description: "Bring up the office connection to activate it",
                expected: [
                    { command: "nmcli", requiredValues: ["connection", "up", "office"] },
                    { command: "nmcli", requiredValues: ["con", "up", "office"] }
                ],
                allowedPreChecks: [],
                explanation: "Connection must be activated to apply all configuration changes.",
                points: 1
            },
            {
                id: 6,
            ...
            },
            {
                ...
                allowedPreChecks: [],
                explanation: "--list-all shows complete firewall configuration.",
                points: 1
            },
            {
                id: 13,
        ...
        set3: [
            ...
            },
            {
                id: 2,
                category: "Implementation",
                description: "Bring down the eth1 connection",
                expected: [
                    { command: "nmcli", requiredValues: ["connection", "down", "eth1"] },
                    { command: "nmcli", requiredValues: ["con", "down", "eth1"] }
                ],
                allowedPreChecks: [],
                explanation: "Connection must be taken down before bringing it back up.",
                points: 1
            },
            {
                id: 3,
                category: "Implementation",
                description: "Bring up the eth1 connection",
                expected: [
                    { command: "nmcli", requiredValues: ["connection", "up", "eth1"] },
                    { command: "nmcli", requiredValues: ["con", "up", "eth1"] }
                ],
                allowedPreChecks: [],
                explanation: "Bringing connection up applies DHCP configuration.",
                points: 1
            },
            {
                id: 4,
            ...
            },
            {
                id: 10,
                category: "Implementation",
                description: "Block port 21 (FTP) permanently",
                expected: [
                    { command: "firewall-cmd", requiredFlags: ["--permanent"], requiredValues: ["--remove-port=21/tcp"] },
                    { command: "firewall-cmd", requiredValues: ["--remove-port=21/tcp", "--permanent"] }
                ],
                allowedPreChecks: [
            ...
            },
            {
                id: 11,
                category: "Implementation",
                description: "Reload firewall to apply changes",
                expected: [
                    { command: "firewall-cmd", requiredValues: ["--reload"] }
                ],
                allowedPreChecks: [],
                explanation: "Firewall must be reloaded after modifying permanent rules.",
                points: 1
            },
            {
                id: 12,
            ...
            },
            {
                id: 14,
                category: "Audit",
                description: "Verify SSH configuration syntax",
                expected: [
                    { command: "sshd", requiredFlags: ["-t"] },
                    { command: "sshd", requiredFlags: ["-T"] }
                ],
                explanation: "sshd -t tests config syntax. -T shows effective configuration.",
                points: 1
            },
            {
                id: 15,
                category: "Implementation",
                description: "Restart sshd service to apply configuration",
                expected: [
                    { command: "systemctl", requiredValues: ["restart", "sshd"] }
                ],
                allowedPreChecks: [],
                explanation: "Service must be restarted to apply sshd_config changes.",
                points: 1
            }
        ],
        ...
        set6: [
            ...
            },
            {
                ...
                expected: [
                    { command: "find", requiredFlags: ["-mtime", "-7", "-exec"], orderedValues: ["/data/", "rsync", "{}", "user@server:/backup/", "\\;"] },
                    { command: "rsync", requiredFlags: ["-av"], requiredValues: ["--files-from=<(find /data/ -mtime -7)", "user@server:/backup/"] }
                ],
                allowedPreChecks: [
                    { command: "find", requiredFlags: ["-mtime", "-7"], requiredValues: ["/data/"] }
                ],
